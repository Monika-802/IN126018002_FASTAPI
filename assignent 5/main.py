...
def sort_products(
    ...
    if sort_by not in valid_sort_fields:
        return {"error": f"sort_by must be one of {valid_sort_fields}"}
    
    if order not in valid_orders:
        return {"error": f"order must be one of {valid_orders}"}

    def get_sort_key(product):
        value = product[sort_by]
...
async def sort_products_by_category():
...
async def browse_products(
    ...
    paged_products = results[start:end]
