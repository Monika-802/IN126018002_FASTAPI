...
def get_product_audit():
    ...
    total_stock_value = sum(p['price'] for p in in_stock_items) * 10
    
    most_expensive_prod = None
    if products:
        max_p = max(products, key=lambda x: x['price'])
        most_expensive_prod = {"name": max_p["name"], "price": max_p["price"]}
    ...
    return {
        ...
        "most_expensive": most_expensive_prod
    }
...
def apply_category_discount(
    ...
    # 1. Filter products by the specified category (case-insensitive)
    for product in products:
        if product['category'].lower() == category.lower():
            
            # 2. Apply the formula: new_price = int(price * (1 - discount_percent / 100))
            discount_factor = 1 - (discount_percent / 100)
            product['price'] = int(product['price'] * discount_factor)
    ...
    # 3. Handle cases where no products match the category
    if not updated_products:
        return {"message": f"No products found in category '{category}'"}

    return {
        "message": f"Successfully applied {discount_percent}% discount to {category} category.",
